    ...
    try:
        async for message in bot.iter_messages(chat, lst_msg_id, CURRENT.get(user_id) if CURRENT.get(user_id) else 0):
            ...
            if message.empty:
                ...
                unsupported += 1
                continue
            elif message.media not in [enums.MessageMediaType.DOCUMENT, enums.MessageMediaType.AUDIO, enums.MessageMediaType.VOICE, enums.MessageMediaType.VIDEO]:  # Non documents and videos files skipping
                unsupported += 1
                continue
            ...
            if not media:
                unsupported += 1
                continue
            elif media.mime_type not in ['document', 'audio', 'voice', 'video']:  # Non mp4 and mkv files types skipping
                unsupported += 1
                continue
            try:
